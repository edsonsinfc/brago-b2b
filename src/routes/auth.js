...
router.post('/login', async (req, res) => {
  ...
  const [rows] = await pool.execute('SELECT id, nome, email, senha, perfil, ativo, COALESCE(equipe_id, NULL) as equipe_id, categoria_acesso, primeiro_login FROM usuarios WHERE email = ? LIMIT 1', [email]);
    ...
    const primeiroLogin = Boolean(usr.primeiro_login);

    const token = jwt.sign({ id: usr.id, nome: usr.nome, email: usr.email, perfil: usr.perfil, equipe_id: usr.equipe_id || undefined, categoria_acesso: usr.categoria_acesso || undefined }, process.env.JWT_SECRET || 'secret', { expiresIn: '8h' });
    
    // Determinar rota de redirecionamento baseado no perfil
    let redirect = '/equipe.html'; // padrão
    if (usr.perfil === 'gestor' || usr.perfil === 'admin') redirect = '/gestor.html';
    if (usr.perfil === 'vendedor') redirect = '/vendedor.html';
    ...
    res.json({ 
      ...
      usuario: { 
        ...
        email: usr.email, 
        perfil: usr.perfil, 
        equipe_id: usr.equipe_id || null, 
        categoria_acesso: usr.categoria_acesso || null 
      } 
    });
  } catch (e) {
    console.error('Erro login:', e);
    // Temporariamente retornando o erro real com status 400 para BYPASSAR a tela HTML do Nginx da Hostinger
    res.status(400).json({ error: `Erro Interno: ${e.message}`, stack: e.stack });
  }
});
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    const resultEmail = await emailService.enviarCodigoRecuperacao(email, codigo, usuario.nome);
    
    if (!resultEmail.success) {
      console.error('❌ Falha ao enviar email:', resultEmail.error);
      // Mesmo com falha no email, retornar sucesso por segurança
      // Mas logar o código no console
      console.log(`⚠️ EMAIL NÃO ENVIADO! Código para ${email}: ${codigo}`);
    }
...
// Rota para trocar senha no primeiro login (requer autenticação)
const { authenticate } = require('../middleware/auth');
router.post('/change-first-password', authenticate, async (req, res) => {
  try {
    ...
    // Verificar se usuário está marcado como primeiro login
    const [rows] = await pool.execute('SELECT primeiro_login FROM usuarios WHERE id = ?', [userId]);
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Usuário não encontrado' });
    }
    ...
    await pool.execute(
      'UPDATE usuarios SET senha = ?, primeiro_login = 0, updated_at = NOW() WHERE id = ?',
      [senhaHash, userId]
    );
    
    console.log(`✅ Primeiro login - senha definida para usuário ID: ${userId}`);
    res.json({ message: 'Senha definida com sucesso' });
  } catch (e) {
    console.error('Erro change-first-password:', e);
    res.status(500).json({ error: 'Erro interno' });
  }
});
